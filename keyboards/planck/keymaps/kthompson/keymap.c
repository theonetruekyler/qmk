...
chord_keys(int num_args, ...)
{
  ...
  // array of keycodes to be read from va_list
  uint16_t *keycode;

  // initialize va_list
  va_list keycode_list;
  va_start(keycode_list, num_args);

  // allocate memory for keycode array
  keycode = malloc((size_t)num_args * sizeof(uint16_t));

  // register keys and then store in array
  for (int i = 0; i < num_args; i++) {
    int arg = va_arg(keycode_list, uint16_t);
    register_code(arg);
    keycode[i] = arg;
  }
